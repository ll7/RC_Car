...
class Joystick:
    ...
    #%% edited by ll7
    # Right stick X axis value scaled between -1.0 (left) and 1.0 (right)
    def rightX_P2500(self,deadzone=0):
        self.refresh()
        raw = int(self.reading[24:30])
        return self.axisScaleP2500_X(raw)
        #return self.axisScale(raw,deadzone)
        
    def rightY_P2500(self,deadzone=0):
        self.refresh()
        raw = int(self.reading[34:40])
        return self.axisScaleP2500_Y(raw)
        #return self.axisScale(raw,deadzone)


    #%% new scales
    def axisScaleP2500_X(self, raw):
        raw = raw/1.0 #int to float
        if abs(raw) >= (32768.0):
            return 0.0
        else:
            if raw >= 0:
                ...
                return (raw + 32768.0) / (32768.0 - 255.0)
            
    def axisScaleP2500_Y(self, raw):
        raw = raw/1.0 
        if raw >= 0:
